...
void memsys_print_stats(MemSys *m)
{
	...
	total += m->mgries_t[i]->s_num_install;
      }
    printf("\nMGRIES_NUM_INSTALL \t : %llu", total);
...
uns64  memsys_dram_access(MemSys *m, Addr lineaddr, uns64 in_cycle, ACTinfo *act_info){
  ...
  //i parse address bits and get my bank
  ...
  dram_get_neighbor_lineaddr(m->mainmem, lineaddr/(m->mainmem->lines_in_rowbuf), &row_prev_lineaddr, &row_next_lineaddr);

  if(act_info != NULL && act_info->isACT) {
    int index = -1;
    Flag outcome = mgries_access(m->mgries_t[act_info->bankID], lineaddr/(m->mainmem->lines_in_rowbuf), &index);
    Flag outcome2 = FALSE, outcome3 = FALSE;
    if(index != -1 && m->mgries_t[act_info->bankID]->entries[index].count == 232) {
      outcome2 = mgries_access(m->mgries_t[act_info->bankID], row_prev_lineaddr/(m->mainmem->lines_in_rowbuf), &index);
      outcome3 = mgries_access(m->mgries_t[act_info->bankID], row_next_lineaddr/(m->mainmem->lines_in_rowbuf), &index);
    }
    if(outcome == TRUE) {
      memsys_rh_mitigate(m, lineaddr/(m->mainmem->lines_in_rowbuf), in_cycle);
    }
    if(outcome2 == TRUE) {
      memsys_rh_mitigate(m, row_prev_lineaddr/(m->mainmem->lines_in_rowbuf), in_cycle);
    }
    if(outcome3 == TRUE) {
      memsys_rh_mitigate(m, row_next_lineaddr/(m->mainmem->lines_in_rowbuf), in_cycle);
    }
  }*/
